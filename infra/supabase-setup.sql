...
-- PostGIS is already enabled in Supabase; uuid-ossp needs manual enabling.
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'trip_status') THEN
        CREATE TYPE trip_status AS ENUM ('Pending','InTransit','DroneHandoff','Completed','Failed');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'cargo_type') THEN
        CREATE TYPE cargo_type AS ENUM ('Organ','Vaccine','Blood','Medication');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'bounty_status') THEN
        CREATE TYPE bounty_status AS ENUM ('Offered','Claimed','Verified','Expired','Rejected');
    END IF;
END
$$;
...
CREATE INDEX IF NOT EXISTS trips_active_idx       ON trips (id) WHERE status IN ('Pending','InTransit','DroneHandoff');
...
CREATE INDEX IF NOT EXISTS corridors_active_idx   ON corridors (trip_id, version DESC) WHERE valid_until IS NULL;
...
CREATE INDEX IF NOT EXISTS webhook_partners_active_idx ON webhook_partners (active) WHERE active = TRUE;
DROP TRIGGER IF EXISTS webhook_partners_set_updated_at ON webhook_partners;
CREATE TRIGGER webhook_partners_set_updated_at BEFORE UPDATE ON webhook_partners FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE INDEX IF NOT EXISTS bounties_open_idx       ON bounties (trip_id, expires_at) WHERE status = 'Offered';

COMMIT;

-- ── seed (update webhook_url to your Render sipra-fleet-receiver URL) ───────
